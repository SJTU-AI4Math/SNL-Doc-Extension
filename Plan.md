...
| 自然专业语言 ⚠️*待定* | PNL? / DNL? | 专业领域内的自然语言，特征是**术语密集**（见 0.B） | 本项目的**输入对象** |
...
| 映射情形 | 名称 | 容忍度 |
|---|---|---|
| 多个术语 → 同一概念 | **冗余 (redundancy)** | 不希望有，但不致命 |
| 一个术语 → 多个概念 | **歧义 (ambiguity)** | **不允许** |
| 一个术语 → 一个概念 | 理想态 | 目标（术语对概念尽量构成单射，最好双射） |

> 一句话：**术语是名字，概念是东西。** 系统必须保证"一个名字只指一样东西"（无歧义），并尽量做到"一样东西只有一个名字"（少冗余）。

### 0.C 操作与载体：把概念落进文档

| 概念 | 缩写 | 含义 |
|---|---|---|
| 术语化 ⚠️*待定* | Terminologization? / Termification? | **从 NL 中抽取**专业概念、为其建立可被系统管理的术语的过程。候选命名各有侧重："化"强调改造、"抽取"强调识别 |
