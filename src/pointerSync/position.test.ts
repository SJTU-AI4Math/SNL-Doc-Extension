...
it('uses the exact UTF16 cursor and lets priority in an expanded scope beat a raw hit', async () => {
  ...
  const prioritized = await buildPointerIndex(root, [
    ...
    { id: 'buffer', pointer: { file: 'x', mode: 'lines', line: 2, beforeLines: 1, afterLines: 0, priority: 0.5 } },
  ]);
  expect(findNearestEntries(prioritized, 'x', 1, 2).candidates.map(x => x.entryId)).toEqual(['buffer']);
  await writePointerIndex(root, prioritized);
  const restored = (await readPointerIndex(root))!;
  expect(findNearestEntries(restored, 'x', 1, 2).candidates.map(x => x.entryId)).toEqual(['buffer']);
  expect(restored.files.x.entries.every(e => !('range' in e.resolution))).toBe(true);
  const stale = { ...restored, version: 1 } as unknown as typeof restored;
