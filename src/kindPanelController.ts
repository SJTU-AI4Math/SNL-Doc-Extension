...
export class KindPanelController {
  ...
  private async pushContext(): Promise<void> {
    ...
    try {
      ...
      void this.panel.webview.postMessage({
        type: 'error',
        message: `Could not load Kind editor data: ${error instanceof Error ? error.message : String(error)}`
      });
    }
  }
      ...
      : result.status === 'duplicate'
        ...
        : result.status === 'notFound'
          ...
          : result.status === 'conflict'
            ? `${cap} kind “${id}” changed after this editor opened. Reload before saving.`
