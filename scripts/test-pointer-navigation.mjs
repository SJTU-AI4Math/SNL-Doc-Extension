...
try{
 for(const shape of ['folder','single'])for(const protocol of ['http','file']){
  ...
  await test('passive-long-entry-keeps-reading-position',async()=>{
    await position(4);await page.keyboard.press('Control+Alt+j');
    await page.waitForFunction(()=>document.querySelector('[data-snl-route-id="a-intro"] [data-snl-source-current]'));
    await page.evaluate(()=>document.querySelector('.snl-export').scrollTop+=600);
    const before=await page.evaluate(()=>({top:document.querySelector('.snl-export').scrollTop,history:history.length,hash:location.hash}));
    await position(4);await page.keyboard.press('ArrowRight');
