...
import { canonicalPath, cleanupOwnedProcessRegistry, destroyOwnedProcessRegistry, ensureOwnedProcessRegistry, fileCensus, requireExternalPath, restoreFiles, sameFileCensus, snapshotFiles, spawnTracked, terminateProcessTree, verifyOwnedProcessRegistryClean } from './library-depth-harness-utils.mjs';
...
if(process.env.SNL_LIBRARY_GEOMETRY_FORCE_FAILURE==='hang-after-start') await new Promise(()=>{});
...
async function cleanupHarness() {
  ...
  try { await cleanupOwnedProcessRegistry(ownershipRegistry); await verifyOwnedProcessRegistryClean(ownershipRegistry); } catch (error) { errors.push(error); }
