...
import { createWorkspaceSvgAssetLoader } from '../render/svgTemplateAssets';
...
export interface SvgEditorDraft {
  source: string; loadedSource: string; template: string; fileName: string;
  assetName: string; accessibilityLabel: string; slotIndex: string;
  exactColor: string; operations: unknown[]; saved: boolean;
}

export interface SvgMacroEditorProps {
  initialDraft?: SvgEditorDraft;
  onDraftChange?(draft: SvgEditorDraft): void;
...
async function expectedProjectionIdentity(slug: string, sourceSvg: string, templateSvg: string, operations: unknown[]): Promise<PendingSave['expected']> {
  const sourceDigest = await digestText(sourceSvg);
  const templateDigest = await digestText(templateSvg);
  ...
  return {
    ...
    manifestPath: `svg/${slug}.manifest.${await digestText(manifest)}.json`
  };
}
...
function assertSplittableAncestors(root: SVGSVGElement, target: SVGElement): void {
  for (let ancestor = target.parentNode as Element | null; ancestor && ancestor !== root; ancestor = ancestor.parentNode as Element | null) {
...
function extractPainterPrefix(root: SVGSVGElement, target: SVGElement): SVGGElement | null {
  ...
  for (let parent = target.parentNode as Element | null; parent && parent !== root; parent = parent.parentNode as Element | null) {
...
export function SvgMacroEditor({ api, editorIdentity, initialProjection, initialDraft, onDraftChange, onTemplateChange, onDirty, onProjectionChange }: SvgMacroEditorProps): React.ReactElement {
  const t = useUiMessages(SVG_EDITOR_MESSAGES);
  const [source, setSource] = useState(initialDraft?.source ?? '');
  const [loadedSource, setLoadedSource] = useState(initialDraft?.loadedSource ?? '');
  const [fileName, setFileName] = useState(initialDraft?.fileName ?? '');
  const [root, setRoot] = useState<SVGSVGElement | null>(() => {
    try { return initialDraft?.template ? parseSanitizedSvgTemplate(initialDraft.template).root : null; } catch { return null; }
  });
  const [selected, setSelected] = useState<SVGGraphicsElement[]>([]);
  const [slotIndex, setSlotIndex] = useState(initialDraft?.slotIndex ?? '0');
  const [error, setError] = useState('');
  const [assetName, setAssetName] = useState(initialDraft?.assetName ?? '');
  const [accessibilityLabel, setAccessibilityLabel] = useState(initialDraft?.accessibilityLabel ?? '');
  const [exactColor, setExactColor] = useState(initialDraft?.exactColor ?? '#000000');
  const [operations, setOperations] = useState<unknown[]>(initialDraft?.operations ?? []);
  const [saved, setSaved] = useState(initialDraft?.saved ?? false);
  const pendingRequest = useRef<PendingSave | null>(null);
  const localEditGeneration = useRef(initialDraft ? 1 : 0);
  const restoredDraft = useRef(Boolean(initialDraft));
  const previousDraft = useRef('');
  useEffect(() => {
    const draft: SvgEditorDraft = { source, loadedSource, fileName, slotIndex, assetName,
      accessibilityLabel, exactColor, operations, saved, template: root ? serialize(root) : '' };
    const encoded = JSON.stringify(draft);
    if (encoded !== previousDraft.current) {
      previousDraft.current = encoded;
      if (localEditGeneration.current > 0) onDraftChange?.(draft);
    }
  }, [source, loadedSource, fileName, slotIndex, assetName, accessibilityLabel, exactColor, operations, saved, root, onDraftChange]);
  ...
  function replaceWithSlot(): void {
    ...
    if (!selected.every((element) => element.parentNode === first.parentNode)) {
      setError(t('slotParents'));
      return;
    }
    try {
      const center = selectionCenter(selected);
      const candidateParent = first.parentNode;
      ...
      const insertionReference = commonParent === first.parentNode
  ...
  async function saveAssets(): Promise<void> {
    ...
    const generation = localEditGeneration.current;
    const expected = await expectedProjectionIdentity(slug, source, templateSvg, operations);
    if (generation !== localEditGeneration.current) { setError(t('changedWhileSaving')); return; }
    pendingRequest.current = { requestId, editorIdentity, slug, label, operationsJson: JSON.stringify(operations), sourceSvg: source, templateSvg, requiredArity, expected };
    setSaved(false);
    setError('');
    host.postMessage({
  ...
  return <section className="snl-svg-macro-editor" aria-label={t('editor')}>
    ...
    <button type="button" disabled={!root || selected.length === 0 || !selected.some((element) => (element.parentNode as Element | null)?.localName === 'g')} onClick={selectParentGroups}>
      {t('selectGroup')}
    </button>
    ...
    <button type="button" disabled={!root || source !== loadedSource} onClick={() => { void saveAssets().catch(() => setError(t('saveFailed'))); }}>{t('save')}</button>
