...
export function BrowserReader({ snapshot }: { snapshot: FrozenReaderSnapshot }): React.ReactElement {
  ...
  const postMessage = (raw: unknown): void => {
    if (!raw || typeof raw !== 'object') return;
    const msg = raw as Record<string, unknown>;
    const asset = frozenAssetReply(snapshot.resources, msg);
    if (asset) { queueMicrotask(() => window.dispatchEvent(new MessageEvent('message', { data: asset }))); return; }
    switch (msg.type) {
  ...
  // The asset hooks call this platform port, not VS Code or browser globals.
  setReaderPlatformApi({ postMessage });
  useEffect(() => {
    ...
    const broker = installWorkspaceAssetBroker({ postMessage });
    ...
    return () => { broker.dispose(); window.removeEventListener('hashchange', changed); window.removeEventListener('popstate', changed); delete host.__snlExportSourceFollow; };
  }, [snapshot]);
  ...
  const markdownImageUrlTransform = (source: string): string => resolveMarkdownAssetUrl(source, 'https://snl-workspace-assets.invalid');
  const sourceAvailable = (id: string): boolean => !!host.__snlSources?.pointers.some(pointer => pointer.entryId === id);
  return <ReaderCapabilitiesContext.Provider value={{ api: { postMessage }, edit: false, graph: false, export: false, sourceAvailable,
    sourceUnavailableReason: 'Source is not included in this frozen export.' }}>
    ...
    {!host.__snlSources ? <p role="note" style={{ padding: '0 1.5rem' }}>Source is not included in this frozen export.</p> : null}
...
export function mountBrowserReader(snapshot: FrozenReaderSnapshot): void {
  ...
  document.documentElement.dataset.snlLogoBlack = document.documentElement.dataset.snlLogoWhite =
    document.querySelector<HTMLImageElement>('.snl-export-watermark img')?.src ?? '';
