...
try {
    ...
    for (const mode of ['plain', 'snl', 'markdown', 'formula', 'svg', 'svg-dark']) {
      ...
      try {
        ...
        if (mode === 'plain') {
          ...
          const measured = await evaluate(`(()=>{const b=document.querySelector('[data-entry-body]'),t=b.querySelector('.snl-text');return{body:[b.clientWidth,b.scrollWidth],text:[t.clientWidth,t.scrollWidth,getComputedStyle(t).overflowWrap,getComputedStyle(t).wordBreak,getComputedStyle(t).fontFamily,getComputedStyle(t).fontSize,getComputedStyle(b).fontSize],root:[document.documentElement.clientWidth,document.documentElement.scrollWidth]}})()`);
          assert(measured.body[0] === measured.body[1] && measured.root[0] === measured.root[1] && measured.text[2] === 'anywhere' && measured.text[3] === 'break-word' && measured.text[4].split(',')[0].replaceAll('\"', '').trim() === 'KaTeX_Main' && Math.abs(parseFloat(measured.text[5])/parseFloat(measured.text[6])-1.21)<0.015, `SNL-TEXT:${width}`, measured);
          evidence.push({ width, mode, measured });
  ...
  for (const width of [320, 240]) {
    ...
    try {
      ...
      const measured = await evaluate(`(()=>{const b=document.querySelector('[data-entry-body]'),t=b.querySelector('.snl-text'),k=t.querySelector('.katex'),m=k.closest('.snl-math-span'),base=k.querySelector('.base'),kr=k.getBoundingClientRect(),br=base.getBoundingClientRect(),walker=document.createTreeWalker(t,NodeFilter.SHOW_TEXT);let prose=null;while(walker.nextNode()){if(walker.currentNode.data.startsWith('MIXEDPROSESENTINEL')){prose=walker.currentNode;break}}const range=document.createRange();range.selectNodeContents(prose);const proseRects=[...range.getClientRects()];const ks=getComputedStyle(k),ts=getComputedStyle(t);return{body:[b.clientWidth,b.scrollWidth,getComputedStyle(b).overflowX,getComputedStyle(b).overflowY],bodyRect:(()=>{const r=b.getBoundingClientRect();return[r.left,r.top,r.right,r.bottom]})(),text:[t.clientWidth,t.scrollWidth,ts.overflowWrap,ts.wordBreak,ts.fontFamily,ts.fontSize,getComputedStyle(b).fontSize],katex:[k.clientWidth,k.scrollWidth,ks.overflowWrap,ks.wordBreak,kr.width,kr.height,ks.fontFamily,ks.fontSize],math:[m?.clientWidth,m?.scrollWidth,m?getComputedStyle(m).overflowX:null,m?.className],panel:(()=>{const p=t.closest('.katex-panel'),r=p?.getBoundingClientRect();return p&&r?[p.clientWidth,p.scrollWidth,getComputedStyle(p).overflowX,getComputedStyle(p).overflowY,r.left,r.top,r.right,r.bottom]:null})(),targets:[...t.closest('.katex-panel')?.querySelectorAll('[data-kind]')??[]].map(e=>{e.classList.add('snl-single-hover','snl-highlight-geometry');const r=e.getBoundingClientRect(),cs=getComputedStyle(e);return[e.dataset.kind,e.className,r.left,r.top,r.right,r.bottom,cs.backgroundColor,cs.boxShadow,cs.outlineStyle,cs.borderStyle,cs.color]}),wheel:(()=>{const target=t.querySelector('[data-kind]')??t,before=b.scrollLeft,event=new WheelEvent('wheel',{deltaY:120,shiftKey:true,bubbles:true,cancelable:true});target.dispatchEvent(event);const after=b.scrollLeft;b.scrollLeft=b.scrollWidth;const tr=target.getBoundingClientRect(),br=b.getBoundingClientRect(),end=[b.scrollLeft,tr.right,br.right,tr.right-1<=br.right];b.scrollLeft=after;return[before,after,event.defaultPrevented,end]})(),base:[br.width,br.height,base.getClientRects().length],proseLines:proseRects.length,proseSpan:proseRects.length?proseRects.at(-1).bottom-proseRects[0].top:0,root:[document.documentElement.clientWidth,document.documentElement.scrollWidth]}})()`);
      assert(
        measured.text[2] === 'anywhere' && measured.text[3] === 'break-word' &&
        measured.text[4].split(',')[0].replaceAll('\"', '').trim() === 'KaTeX_Main' &&
        Math.abs(parseFloat(measured.text[5])/parseFloat(measured.text[6])-1.21)<0.015 &&
        measured.katex[6].includes('KaTeX_Main') && Math.abs(parseFloat(measured.katex[7])-parseFloat(measured.text[5]))<0.1 &&
        ...
        measured.body[1] > measured.body[0] && measured.body[2] === 'auto' && measured.body[3] === 'hidden' && measured.math[2] === 'visible' &&
        measured.panel[2] === 'visible' && measured.panel[3] === 'visible' &&
        measured.targets.length > 0 && measured.targets[0][6] === 'rgba(0, 0, 0, 0)' &&
        measured.targets[0][7] === 'none' && measured.targets[0][8] === 'none' && measured.targets[0][9] === 'none' &&
        measured.wheel[1] > measured.wheel[0] && measured.wheel[2] === true && measured.wheel[3][3] === true &&
        measured.targets[0][3] - 1 < measured.panel[5] &&
        measured.targets[0][3] - 1 >= measured.bodyRect[1] && measured.targets[0][5] + 1 <= measured.bodyRect[3] &&
  ...
  for (const width of [320, 480, 1000]) {
    ...
    try {
      ...
      assert(
        ...
        measured.body[0] === measured.body[1] && measured.body[2] === 'auto' && measured.body[3] === 'hidden' && measured.body[4] === measured.body[5] &&
        measured.panel[2] === 'visible' && measured.panel[3] === 'visible' && measured.panel[4] === measured.panel[5] &&
        measured.root[0] === measured.root[1],
        `ROOT-TEXT-CM:${width}`,
        measured
      );
  ...
  for (const { width, height, rootPaddingTop = 0 } of viewportScenarios) {
    ...
    try {
      ...
      // Dashboard/header growth may place the origin below a short viewport.
      // Scroll as a user would before sending native pointer coordinates; never
      // bypass the existing hover/pin/hit-test assertions with synthetic events.
      await evaluate(`(()=>{document.querySelector(${JSON.stringify(rootSelector)}).scrollIntoView({behavior:'instant',block:'nearest'});return new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)))})()`);
      const rootReady = await evaluate(`(()=>{const e=document.querySelector(${JSON.stringify(rootSelector)}),r=e.getBoundingClientRect();return{visible:r.top>=0&&r.bottom<=innerHeight,hit:e.contains(document.elementFromPoint(r.left+r.width/2,r.top+r.height/2))}})()`);
      assert(rootReady.visible && rootReady.hit, `ORIGIN-READY-ROOT:${width}x${height}`, rootReady);
