...
try {
    ...
    for (const mode of ['plain', 'snl', 'markdown', 'formula']) {
      ...
      try {
        ...
        if (mode === 'plain') {
          ...
          const measured = await evaluate(`(()=>{const b=document.querySelector('[data-entry-body]'),t=b.querySelector('.snl-text'),bs=getComputedStyle(b),ts=getComputedStyle(t);return{body:[b.clientWidth,b.scrollWidth,bs.fontSize],text:[t.clientWidth,t.scrollWidth,ts.overflowWrap,ts.wordBreak,ts.fontFamily,ts.fontSize],root:[document.documentElement.clientWidth,document.documentElement.scrollWidth]}})()`);
          assert(measured.body[0] === measured.body[1] && measured.root[0] === measured.root[1] && measured.text[2] === 'anywhere' && measured.text[3] === 'break-word' && measured.text[4].split(',')[0].replaceAll('\"', '').trim() === 'KaTeX_Main' && measured.text[5] === measured.body[2], `SNL-TEXT:${width}`, measured);
          evidence.push({ width, mode, measured });
          ...
          const measured = await evaluate(`(()=>{const b=document.querySelector('[data-entry-body]'),p=b.querySelector('.snl-markdown-body p'),pre=b.querySelector('.snl-markdown-body pre'),ps=getComputedStyle(pre);return{body:[b.clientWidth,b.scrollWidth],prose:[p.clientWidth,p.scrollWidth,getComputedStyle(p).overflowWrap,getComputedStyle(p).wordBreak],code:[pre.clientWidth,pre.scrollWidth,ps.overflowX,ps.whiteSpace,ps.fontFamily],root:[document.documentElement.clientWidth,document.documentElement.scrollWidth]}})()`);
  ...
  for (const width of [320, 240]) {
    ...
    try {
      ...
      const measured = await evaluate(`(()=>{const b=document.querySelector('[data-entry-body]'),t=b.querySelector('.snl-text'),k=t.querySelector('.katex'),base=k.querySelector('.base'),kr=k.getBoundingClientRect(),br=base.getBoundingClientRect(),walker=document.createTreeWalker(t,NodeFilter.SHOW_TEXT);let prose=null;while(walker.nextNode()){if(walker.currentNode.data.startsWith('MIXEDPROSESENTINEL')){prose=walker.currentNode;break}}const range=document.createRange();range.selectNodeContents(prose);const proseRects=[...range.getClientRects()];const bs=getComputedStyle(b),ks=getComputedStyle(k),ts=getComputedStyle(t);return{body:[b.clientWidth,b.scrollWidth,bs.overflowX,bs.fontSize],text:[t.clientWidth,t.scrollWidth,ts.overflowWrap,ts.wordBreak,ts.fontFamily,ts.fontSize],katex:[k.clientWidth,k.scrollWidth,ks.overflowWrap,ks.wordBreak,kr.width,kr.height,ks.fontFamily,ks.fontSize],base:[br.width,br.height,base.getClientRects().length],proseLines:proseRects.length,proseSpan:proseRects.length?proseRects.at(-1).bottom-proseRects[0].top:0,root:[document.documentElement.clientWidth,document.documentElement.scrollWidth]}})()`);
      assert(
        measured.text[2] === 'anywhere' && measured.text[3] === 'break-word' &&
        measured.text[4].split(',')[0].replaceAll('\"', '').trim() === 'KaTeX_Main' && measured.text[5] === measured.body[3] &&
        measured.katex[2] === 'normal' && measured.katex[3] === 'normal' &&
        measured.katex[4] > measured.body[0] && measured.katex[5] < 40 && measured.katex[6].includes('KaTeX_Main') && measured.katex[7] === measured.text[5] &&
