...
const M = defineUiMessages('sourceExportOptions', {
  ...
  scope: 'Source scope', pointers: 'Pointer files only (complete files)', project: 'Filtered project snapshot (not necessarily buildable)', keep: 'Keep rules (override defaults, one per line)', exclude: 'Exclude rules (win over keep rules)', companion: 'Additional license / notice files', external: 'Explicitly authorize external dependency roots (absolute directories)', fileLimit: 'Maximum file bytes', totalLimit: 'Maximum total source bytes', missing: 'Explicitly accept unavailable Pointer targets', mathlib: 'Use Mathlib example rules', available: 'Refresh and recapture the Library to enable source snapshots.', note: 'Rules use root-relative / paths, * and **. Defaults exclude .SNL_Doc, .git, .lake, build output and common credentials. Output is always excluded.'
}, {
  enable: '包含源代码（只读 Monaco）', warning: '每一位 HTML 阅读者都会收到这些源码字节。只读不是保密措施；导出前请检查凭据与依赖许可。',
  scope: '源码范围', pointers: '仅 Pointer 涉及的完整文件', project: '筛选后的完整项目结构（不保证可编译）', keep: '保留规则（覆盖默认排除，每行一条）', exclude: '排除规则（优先于保留规则）', companion: '额外许可 / NOTICE 伴随文件', external: '明确授权的外部依赖根（绝对目录）', fileLimit: '每文件字节上限', totalLimit: '源码总字节上限', missing: '明确接受 Pointer 目标源码缺失', mathlib: '填入 Mathlib 示例规则', available: '请刷新并重新捕获 Library，以启用源码快照。', note: '规则为根相对 / 路径，支持 * 和 **。默认排除 .SNL_Doc、.git、.lake、构建产物及常见凭据；当前输出始终排除。'
});
