...
let replaceThenFailWritePath: string | null = null;
  ...
  workspace: {
    fs: {
      ...
      writeFile: vi.fn(async (target: MemUri, value: Uint8Array) => {
        ...
        if (target.fsPath === replaceThenFailWritePath) {
          replaceThenFailWritePath = null;
          files.set(target.fsPath, enc.encode(JSON.stringify({ external: true })));
...
describe('updateLibraryDraft three-file transaction', () => {
  ...
  it('leaves a complete current-file residue when backend ownership is ambiguous after I/O starts', async () => {
    const before = { meta: json(metaPath), graph: json(graphPath), counters: json(countersPath) };
    const candidate = payload();
    failAfterWritePath = countersPath;
    expect(await updateLibraryDraft(root, 'lib', candidate)).toMatchObject({
      status: 'error', message: expect.stringContaining('rollback was incomplete')
    });
    expect(json(metaPath)).toEqual(before.meta);
    expect(json(graphPath)).toEqual(before.graph);
    expect(json(countersPath)).toMatchObject({
      counters: [{ id: 'counter-a', name: 'renamed', numbering: 'I' }]
    });
  });

  it('fails closed without overwriting an ambiguous truncated current file', async () => {
    ...
    expect(files.get(metaPath)).toEqual(before.get(metaPath));
    expect(files.get(graphPath)).toEqual(before.get(graphPath));
    expect(files.get(countersPath)).not.toEqual(before.get(countersPath));
  });

  it('does not delete an external replacement while compensating a failed optional-file creation', async () => {
    files.delete(countersPath);
    const input = {
    ...
    replaceThenFailWritePath = countersPath;
    expect(await updateLibraryDraft(root, 'lib', input)).toMatchObject({
      status: 'error', message: expect.stringContaining('rollback was incomplete')
    });
    expect(json(metaPath).title).toBe('Old');
    expect(json(countersPath)).toEqual({ external: true });
  });

  it('does not overwrite an external replacement after this write enters the backend and fails', async () => {
    replaceThenFailWritePath = countersPath;
    expect(await updateLibraryDraft(root, 'lib', payload())).toMatchObject({
      status: 'error', message: expect.stringContaining('rollback was incomplete')
    });
    expect(json(metaPath).title).toBe('Old');
    expect(json(graphPath).graphExtension).toEqual({ keep: true });
    expect(json(countersPath)).toEqual({ external: true });
  });
});
