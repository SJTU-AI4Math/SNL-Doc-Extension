...
afterEach(cleanup);
Object.defineProperty(crypto, 'subtle', { configurable: true, value: { digest: async (_algorithm: string, data: Uint8Array) => { const hash = createHash('sha256').update(data).digest(); return hash.buffer.slice(hash.byteOffset, hash.byteOffset + hash.byteLength); } } });
...
function projectionFor(request: Record<string, unknown>): Record<string, unknown> {
  ...
  const sourceSvg = request.sourceSvg as string;
  const templateSvg = request.templateSvg as string;
  const operations = request.operations as unknown[];
  const sourceDigest = digest(sourceSvg);
  const templateDigest = digest(templateSvg);
  const source = `svg/${slug}.source.${sourceDigest}.svg`;
  const template = `svg/${slug}.template.${templateDigest}.svg`;
  const manifestText = `${JSON.stringify({
    version: 1, compiler: 'snl-doc-extension-svg-editor:v1', source,
    source_revision: `sha256:${sourceDigest}`, output: template,
    output_revision: `sha256:${templateDigest}`, operations
  }, null, 2)}\n`;
  return {
...
describe('SvgMacroEditor', () => {
  it('localizes the editor controls in Chinese', async () => {
  ...
  it('imports safe SVG, selects a painted occurrence, and replaces it with a slot', async () => {
  ...
  it('stores a root-level slot anchor in SVG user coordinates under CSS viewport scaling', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
    fireEvent.change(screen.getByLabelText('SVG source'), { target: { value: RAW } });
    fireEvent.click(screen.getByRole('button', { name: 'Load SVG preview' }));
  ...
  it('places a multi-selection slot at the earliest selected painter position, not click order', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
  ...
  it('fails closed when a multi-selection spans different painter parents', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
  ...
  it('replaces one painted use occurrence without deleting its shared definition or sibling use', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
  ...
  it('cannot save a template compiled from a stale preview after source edits', async () => {
  ...
  it('saves source, compiled template, and operations through the host then returns the projection', async () => {
    ...
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: 'Save SVG Macro Asset' })); });
    ...
    const projection = projectionFor(posted.at(-1) as Record<string, unknown>);
  ...
  it('rejects malformed or stale save replies without patching the Macro draft', async () => {
    const posted: Record<string, unknown>[] = [];
    const onProjectionChange = vi.fn();
    ...
    fireEvent.change(screen.getByLabelText('SVG source'), { target: { value: RAW.replace('#000000', '#ff0000') } });
    const validProjection = projectionFor(posted.at(-1) as Record<string, unknown>);
  ...
  it('does not adopt a save after projection-affecting controls change', async () => {
    const posted: Record<string, unknown>[] = [];
    const onProjectionChange = vi.fn();
    render(<SvgMacroEditor api={{ postMessage: (message) => posted.push(message as Record<string, unknown>) }} onTemplateChange={() => {}} onProjectionChange={onProjectionChange} />);
    fireEvent.change(screen.getByLabelText('SVG source'), { target: { value: RAW } });
  ...
  it('rejects internally inconsistent projection identities', async () => {
    const posted: Record<string, unknown>[] = [];
    const onProjectionChange = vi.fn();
    render(<SvgMacroEditor api={{ postMessage: (message) => posted.push(message as Record<string, unknown>) }} onTemplateChange={() => {}} onProjectionChange={onProjectionChange} />);
    fireEvent.change(screen.getByLabelText('SVG source'), { target: { value: RAW } });
  ...
  it('rejects a self-consistent projection whose digests do not match the submitted bytes', async () => {
    const posted: Record<string, unknown>[] = [];
    const onProjectionChange = vi.fn();
    render(<SvgMacroEditor api={{ postMessage: (message) => posted.push(message as Record<string, unknown>) }} onTemplateChange={() => {}} onProjectionChange={onProjectionChange} />);
    fireEvent.change(screen.getByLabelText('SVG source'), { target: { value: RAW } });
    ...
    const forged = projectionFor(request) as {
      asset: { source: string; revision: string };
      editor: { source: string; source_revision: string; manifest: string };
    };
    forged.asset.source = `svg/diagram.template.${'d'.repeat(64)}.svg`;
    forged.asset.revision = `sha256:${'d'.repeat(64)}`;
    forged.editor.source = `svg/diagram.source.${'e'.repeat(64)}.svg`;
    forged.editor.source_revision = `sha256:${'e'.repeat(64)}`;
    forged.editor.manifest = `svg/diagram.manifest.${'f'.repeat(64)}.json`;
    fireEvent(window, new MessageEvent('message', { data: { type: 'svgMacro.assetsWritten', requestId: request.requestId, projection: forged } }));
    expect(onProjectionChange).not.toHaveBeenCalled();
    expect(screen.getByRole('alert').textContent).toMatch(/invalid/i);
  });

  it('does not adopt a pending save reply into a different style identity', async () => {
    const posted: Record<string, unknown>[] = [];
  ...
  it('hydrates existing raw source and runtime template through immutable asset identities', async () => {
    ...
    for (const call of postMessage.mock.calls) {
      ...
      window.dispatchEvent(new MessageEvent('message', { data: {
        type: 'snl.assets/svg-source', request_id: request.request_id,
        source: request.source, base_identity: request.base_identity, revision: request.revision,
        value: request.source.includes('.source.') ? RAW : RAW.replace('<circle', '<rect x="60" y="20" width="10" height="10"/><circle')
      } }));
    }
    await waitFor(() => expect((screen.getByLabelText('SVG source') as HTMLTextAreaElement).value).toBe(RAW));
  ...
  it('keeps controls and the visual canvas in stable grid columns', async () => {
    render(<SvgMacroEditor onTemplateChange={() => {}} />);
  ...
  it('rejects unsafe raw SVG before mounting a preview', async () => {
    render(<SvgMacroEditor onTemplateChange={() => {}} />);
  ...
  it('rolls back the whole paper-key compilation when a later target is unsupported', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
  ...
  it('ascends from a painted occurrence to replace its authored group', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
  ...
  it('maps an exact authored color key across painted occurrences', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
  ...
  it('does not rewrite a shared definition as though it were a painted paper occurrence', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
  ...
  it('compiles an exact paper color into an ordered full-viewBox knockout', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
  ...
  it('maps the selected painted channels to inherited foreground', async () => {
    const onTemplateChange = vi.fn();
    render(<SvgMacroEditor onTemplateChange={onTemplateChange} />);
    fireEvent.change(screen.getByLabelText('SVG source'), { target: { value: RAW } });
    fireEvent.click(screen.getByRole('button', { name: 'Load SVG preview' }));
