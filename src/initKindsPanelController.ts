...
const MESSAGES = defineHostMessages(
  ...
  {
    entryTitle: 'SNL 初始化条目类型', macroTitle: 'SNL 初始化宏类型', noWorkspace: '{title}需要打开文件夹或工作区。', noPreset: '未选择预设。', applied: '已应用预设“{presetId}”——添加了 {count} 个{kind}。', nonEmpty: '{configKey} 已有 {count} 个条目。预设只能初始化空目录。', initFirst: '.SNL_Doc 尚不存在。请先运行“SNL: Init”。', unknownPreset: '未知预设 ID：{presetId}', failed: '{title}失败：{error}', entryKind: '条目类型', macroKind: '宏类型', fulcrumLabel: 'Fulcrum 数学笔记', fulcrumDescription: '章节层级结构以及 Fulcrum 数学内容类型。', leanLabel: 'Lean 4 文档', leanDescription: '用于映射 Lean 4 代码的文档条目类型（占位）。', tsLabel: 'TypeScript 文档', tsDescription: '用于映射 TypeScript 代码的文档条目类型（占位）。', pythonLabel: 'Python 文档', pythonDescription: '用于映射 Python 代码的文档条目类型（占位）。', basicsLabel: 'SNL-Basics 默认类型', basicsDescription: 'SNL-Basics 默认宏类型，以及用于辅助子树的 partial 类型。', mathematicsLabel: '数学写作', mathematicsDescription: '适用于广泛数学写作的条目类型集合。'
  }
);
...
export function projectKindPresets(
  ...
  return source.map((preset) => {
    const keys = (PRESET_COPY_KEYS[domain] as Record<string, readonly [keyof typeof MESSAGES.en, keyof typeof MESSAGES.en]>)[preset.id];
    return {
      id: preset.id,
