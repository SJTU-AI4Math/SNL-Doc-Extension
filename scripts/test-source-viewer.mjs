...
const p=(entryId,line,endLine=line)=>{const pointer={mode:'lines',file:'Main.lean',line,endLine,...(entryId==='Demo'?{beforeLines:0,afterLines:0,column:1,endColumn:2}:{})};const range={startLine:line,startColumn:1,endLine,endColumn:2,coveredEndLine:endLine};return {entryId,pointer,fileId:'f-one',sourceSha256:sha(bytes),range,inverseScope:compilePointerScope(pointer,range,text),status:'ok'};};
...
for (const [mode,url] of [['http',`http://127.0.0.1:${server.address().port}/directory.html`],['directory-file',pathToFileURL(resolve(dir,'directory.html')).href],['inline-file',pathToFileURL(resolve(dir,'inline.html')).href]]) {
  ...
  const reveal=await page.evaluate(()=>{const e=window.monaco.editor.getEditors()[0],s=e.getSelection();return {active:e.getPosition(),anchor:s.getSelectionStart(),range:[s.startLineNumber,s.startColumn,s.endLineNumber,s.endColumn],text:e.getModel().getValueInRange(s)};});
  assert.deepEqual(reveal.active,{lineNumber:1,column:1},'programmatic reveal caret must be inside the half-open scope');
  assert.deepEqual(reveal.anchor,{lineNumber:6,column:2});assert.deepEqual(reveal.range,[1,1,6,2]);
  assert.equal(reveal.text,text.slice(0,text.indexOf('  rfl')+1),'complete original selection retained');
  await page.getByRole('button',{name:'Nearby entry (Ctrl+Alt+J)',exact:true}).click();
  await page.waitForSelector('[data-snl-source-current][data-entry-id="Demo"]');
  assert.equal(await page.evaluate(()=>location.hash),'#/node/node-demo');
  await page.goBack();await page.waitForFunction(()=>location.hash==='');
  await page.evaluate(()=>window.monaco.editor.getEditors()[0].setPosition({lineNumber:6,column:2},'test-program'));
  await page.getByRole('button',{name:'Nearby entry (Ctrl+Alt+J)',exact:true}).click();
  assert.equal(await page.locator('.snl-source-status').textContent(),'No nearby entry');
  assert.equal(await page.locator('[data-snl-source-current]').count(),0,'excluded endpoint must stay excluded');
  await page.locator('[data-snl-source-entry="Demo"]').click();
  await page.waitForFunction(()=>window.monaco.editor.getEditors()[0].getPosition().lineNumber===1);
