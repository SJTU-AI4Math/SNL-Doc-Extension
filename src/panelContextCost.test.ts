...
vi.mock('vscode', () => {
  ...
  const payloadFor = (name: string): string => {
    if (name === 'config.json') {
      return JSON.stringify({ active_macro_packages: PACKAGES });
    }
  ...
  return {
    ...
    workspace: {
      ...
      fs: {
        readDirectory: async () =>
          PACKAGES.map((name) => [`${name}.json`, file] as [string, number]),
...
describe('panel context cost', () => {
  it('reads each macro package exactly once', async () => {
    ...
    expect(Object.keys(macros)).toHaveLength(PACKAGES.length * 2);
