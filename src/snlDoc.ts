      ...
      for (const { operation, completed, ioStarted, originalBytes } of attempted.reverse()) {
        try {
          if (!completed) {
            ...
            // Once backend I/O has started and then failed, pathname ownership
            // is ambiguous: identical bytes do not prove that the current file
            // is still ours. Leave the current target untouched and surface an
            // incomplete rollback instead of overwriting a possible replacement.
            throw new Error('failed operation reached backend I/O; target ownership is ambiguous and was left untouched');
          }
          if (operation.kind === 'write') {
...
export async function updateLibraryDraft(
  ...
  return withExtensionWriterLock(workspaceRoot, 'save complete Library draft', async () => {
    ...
    try {
      ...
      const validateCounters = (nodes: CounterNode[]): boolean => nodes.every((node) => {
        ...
        return validateCounters(node.children);
      });
