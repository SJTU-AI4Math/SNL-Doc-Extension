...
describe('filter refresh and panel lifecycle', () => {
  ...
  it('localizes accessible groups including Tags in Chinese, and changes kind to an explicitly empty draft', () => {
    ...
    expect(within(select).getAllByRole('option').map(option => option.textContent)).toEqual(['条目种类', '关系', '标签']);
    check(card, 'theorem');
...
it('filters exact canonical tags with AND/OR, empty identity, complete choices and unavailable recovery', () => {
  const data = {...graph, nodes: [
    {...node('a','theorem'),tags:['','__proto__',' A ','same','same']},
    {...node('b','lemma'),tags:[' a,b ','中文','__proto__']},
    {...node('c','definition'),tags:['A','same']}
  ]};
  mount(data);
  const first = add();
  expect(within(first).getByRole('option',{name:'Tag'})).toBeTruthy();
  fireEvent.change(within(first).getByRole('combobox',{name:'Filter kind'}),{target:{value:'tag'}});
  check(first,'(empty tag)'); expect(ids()).toEqual(['a']);
  check(first,'中文'); expect(ids()).toEqual(['a','b']);
  expect([...first.querySelectorAll<HTMLInputElement>('input[data-testid="graph-filter-value"]')].map(n=>n.value)).toEqual(expect.arrayContaining(['A',' A ',' a,b ']));
  const second = add();
  fireEvent.change(within(second).getByRole('combobox',{name:'Filter kind'}),{target:{value:'tag'}});
  check(second,'same'); expect(ids()).toEqual(['a']);
  check(second,'__proto__'); expect(ids()).toEqual(['a','b']);
  fireEvent.click(within(first).getByRole('checkbox',{name:'Enabled'})); expect(ids()).toEqual(['a','b','c']);
  fireEvent.click(within(first).getByRole('checkbox',{name:'Enabled'}));
  send({...data,nodes:data.nodes.map(n=>({...n,tags:[]}))}); expect(ids()).toEqual([]);
  expect(within(first).getByRole('checkbox',{name:'(empty tag) (unavailable)'})).toBeTruthy();
  fireEvent.click(screen.getByRole('button',{name:'Clear filters'}));expect(ids()).toEqual(['a','b','c']);
  expect(api.setState).not.toHaveBeenCalled();
});
it('honors the explicit Reader atomic-only opt-out without changing ordinary panel defaults', () => {
  render(<SnlGraphApp initialAtomicDependenciesOnly={false} />);
  send({...graph,edges:[edge('aa','a','a','composite',true,false)]});
  expect(ids()).toEqual(['a']);
});
