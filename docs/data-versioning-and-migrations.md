...
- `0.0.7 -> 0.0.8`: validate each flat Entry/Macro Kind color pair, then split it losslessly into explicit `light` and `dark` `{ stroke, background }` variants while preserving unknown coloring extensions.
