...
export async function verifyGraphScale({ evaluate, wait, screenshot, page, evidence }) {
  ...
  for(const mode of ['rectangle','radial-outward','radial-inward']) for(const packing of (mode==='rectangle'?['bands']:['bands','rings'])) {
    ...
    for(const target of [.15,.3,.8,2]) {
      ...
      for(const n of s.nodes){close(n.r,12,'world dot radius');close(n.stroke,2,'world outline');close(n.w,24*s.scale,'screen diameter');}
    ...
    const larger=await zoom(.8,true);const ratio=larger.scale/active.scale;
    const titlePairs=larger.nodes.filter((n,i)=>n.shape==='title'&&active.nodes[i].shape==='title');assert.ok(titlePairs.some(n=>n.id==='Goal'));
    larger.nodes.forEach((n,i)=>{if(n.shape!=='title'||active.nodes[i].shape!=='title')return;close(n.w,active.nodes[i].w*ratio,'card width follows zoom');close(n.h,active.nodes[i].h*ratio,'card height follows zoom');assert.ok(n.glyphH>0);close(n.glyphH,active.nodes[i].glyphH*ratio,'title text follows zoom');});
