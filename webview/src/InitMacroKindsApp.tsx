// SNL Initialize Macro Kinds webview: a preset picker for seeding
// `config.json#macro_kinds` from scratch. Refuses when the catalog already
// has entries — see InitMacroKindsPanel for the reason.
...
export function InitMacroKindsApp(): React.ReactElement {
  ...
  return (
    <main style={{ ...PANEL_STYLE, maxWidth: '40rem' }}>
      ...
      <p style={{ margin: '0 0 1rem', opacity: 0.85 }}>
        Seed <code>.SNL_Doc/config.json#macro_kinds</code> from a preset.
        Presets replace an empty catalog only — if you already have macro
        kinds, edit them individually via <strong>Create Macro Kind</strong>{' '}
      ...
      {catalogBusy ? (
        ...
        >
          ⚠️ macro_kinds already has {existing} entr
...
function StatusLine({
  ...
  if (status.kind === 'applied') {
    ...
    text = `✅ Applied "${label}" — ${status.count} macro kind${
      status.count === 1 ? '' : 's'
    } added.`;
